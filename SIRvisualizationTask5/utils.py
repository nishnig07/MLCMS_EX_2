...
def file_df_to_count_df(df,
    ...
    for pid in pedestrian_ids:
        ...
        for (st, g) in simtime_group:
            ...
            if g == ID_CURED and g != current_state and current_state == ID_INFECTED:
                current_state = g
                group_counts.loc[group_counts['simTime'] > st, 'group-i'] -= 1
                group_counts.loc[group_counts['simTime'] > st, 'group-c'] += 1
...
def create_folder_data_scatter(folder):
    ...
    group_counts = file_df_to_count_df(data, ID_INFECTED=ID_INFECTED, ID_SUSCEPTIBLE=ID_SUSCEPTIBLE, ID_CURED=ID_CURED)
    group_counts.plot()
    ...
    scatter_i = go.Scatter(x=group_counts['simTime'],
                           ...
                           mode='lines')
    scatter_c = go.Scatter(x=group_counts['simTime'],
                           y=group_counts['group-c'],
                           name='recovered' + os.path.basename(folder),
                           mode='lines')
    return [scatter_s, scatter_i, scatter_c], group_counts
