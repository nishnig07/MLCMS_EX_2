...
public class SIRGroupModel extends AbstractGroupModel<SIRGroup> {
	...
	private LinkedCellsGrid<Pedestrian> linkedCellsGrid;
	private double sideLength = 10.0;
	private double lastSimTimeInSec = 0.0;
	...
	public void initialize(List<Attributes> attributesList, Domain domain,
		...
		this.linkedCellsGrid = new LinkedCellsGrid<>(new VRectangle(topography.getBounds()), sideLength);
	}
	...
	public void update(final double simTimeInSec) {
		...
		if (simTimeInSec - this.lastSimTimeInSec >=1) {

			DynamicElementContainer<Pedestrian> c = topography.getPedestrianDynamicElements();

			linkedCellsGrid.clear();
			...
			if (c.getElements().size() > 0) {
				for (Pedestrian p : c.getElements()) {
					for (Pedestrian p_neighbor : linkedCellsGrid.getObjects(p.getPosition(), attributesSIRG.getInfectionMaxDistance())) {
						if (p == p_neighbor || getGroup(p_neighbor).getID() != SIRType.ID_INFECTED.ordinal() || getGroup(p).getID() == SIRType.ID_CURED.ordinal())
							continue;
						...
						else if (this.random.nextDouble() < attributesSIRG.getRecoveryRate()){
							SIRGroup g = getGroup(p);
							if (g.getID() == SIRType.ID_INFECTED.ordinal()){
								elementRemoved(p);
								assignToGroup(p, SIRType.ID_CURED.ordinal());
							}
						}
					}
				}
